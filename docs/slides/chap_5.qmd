...
[Remission times (before relapsing), in weeks, from a clinical trial of **42 leukaemia patients**]{.alert}. Patients matched in pairs and randomised to 6-mercaptopurine (drug) or control. In `R`, data is found in the `MASS` package in the `gehan` object.
